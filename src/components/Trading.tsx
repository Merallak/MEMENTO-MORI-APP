...
import { FileQuestion, ArrowRight, TrendingUp, TrendingDown } from "lucide-react";
...
function PriceChart({ currentPrice, ticker, tokenId }: { currentPrice: number; ticker: string; tokenId: string }) {
  ...
  return (
    <div className="relative w-full h-[220px] bg-muted/20 rounded-lg p-4 border border-border/50">
      <div className="flex justify-between items-start mb-2">
        <div>
          <div className="text-sm text-muted-foreground">{ticker} / USD</div>
          <div className="text-2xl font-bold font-mono">${currentPrice.toFixed(4)}</div>
        </div>
        <div className={`flex items-center gap-1 px-2 py-1 rounded-md ${isPositive ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>
          {isPositive ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
          <span className="text-sm font-mono font-bold">{percentChange > 0 ? "+" : ""}{percentChange.toFixed(2)}%</span>
        </div>
      </div>
      <svg 
      ...
      >
        {/* Grid lines */}
        <line x1="0" y1={svgHeight * 0.25} x2={svgWidth} y2={svgHeight * 0.25} stroke="currentColor" strokeWidth="0.5" className="text-border opacity-30" />
        <line x1="0" y1={svgHeight * 0.5} x2={svgWidth} y2={svgHeight * 0.5} stroke="currentColor" strokeWidth="0.5" className="text-border opacity-30" />
        <line x1="0" y1={svgHeight * 0.75} x2={svgWidth} y2={svgHeight * 0.75} stroke="currentColor" strokeWidth="0.5" className="text-border opacity-30" />
        
        {/* Gradient fill */}
        <defs>
          <linearGradient id={`priceGradient-${tokenId}`} x1="0%" y1="0%" x2="0%" y2="100%">
...
export function Trading() {
  ...
  const handleOrder = async (type: "buy" | "sell") => {
    ...
    if (type === "buy") {
      ...
      if (!validation.valid) {
        toast({ title: t("common.error"), description: validation.message, variant: "destructive" });
        return;
      }
    } else {
      const validation = await DataService.validateSellOrder(user.id, selectedToken.id, numAmount);
      if (!validation.valid) {
        toast({ title: t("common.error"), description: validation.message, variant: "destructive" });
        return;
      }
    ...
    let success = false;
    if (type === "buy") {
      success = await DataService.executeBuyOrder(user.id, selectedToken.id, numAmount, numPrice);
    } else {
      success = await DataService.executeSellOrder(user.id, selectedToken.id, numAmount, numPrice);
    }

    if (success) {
      toast({ title: t("common.success"), description: `${type === "buy" ? "Purchase" : "Sale"} completed successfully` });
      setBuyAmount(0);
      setBuyPrice(selectedToken.current_price);
      setSellAmount(0);
      setSellPrice(selectedToken.current_price);
      ...
      toast({ title: t("common.error"), description: "Failed to complete transaction", variant: "destructive" });
    }
  };
  ...
  const handleSwapOffer = async () => {
    ...
    // Corrección: price debe ser receiveAmount / payAmount
    // Esto representa cuánto del token de pago se necesita por unidad del token objetivo
    ...
    if (order) {
      toast({ title: t("common.success"), description: "Swap Offer created successfully" });
      setReceiveAmount(0);
      setPayAmount(0);
      loadOrderBook(selectedToken.id);
    } else {
      toast({ title: t("common.error"), description: "Failed to create swap offer", variant: "destructive" });
    }
  };
  ...
  const handleAcceptSwap = async (offerId: string) => {
    ...
    if (result.success) {
      toast({ title: t("common.success"), description: t("trading.swap.success_msg") || "Swap executed successfully!" });
      loadOrderBook(selectedToken!.id);
      loadData();
    } else {
  ...
  const handleCancelOrder = async (orderId: string) => {
    ...
    if (success) {
      toast({ title: t("common.success"), description: "Order cancelled successfully" });
      if (selectedToken) loadOrderBook(selectedToken.id);
    } else {
      toast({ title: t("common.error"), description: "Failed to cancel order", variant: "destructive" });
    }
  };

  if (!selectedToken) return <div className="p-8 text-center text-muted-foreground">{t("trading.loading_market")}</div>;
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-8 space-y-4">
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <Avatar className="w-16 h-16 border-2 border-border">
                    <AvatarImage src={selectedToken.image_url || ""} className="object-cover" />
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-3xl font-display font-bold text-foreground">{selectedToken.ticker}</span>
                      <span className="text-lg text-muted-foreground font-sans">{selectedToken.name}</span>
                    </div>
                    <p className="mt-1 text-sm text-muted-foreground">{selectedToken.description}</p>
                  </div>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-3 rounded-lg bg-muted/30 border border-border">
                  <div className="text-xs text-muted-foreground uppercase tracking-wider">{t("market.market_cap")}</div>
                  <div className="font-bold mt-1 font-mono text-foreground">${selectedToken.market_cap.toLocaleString()}</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/30 border border-border">
                  <div className="text-xs text-muted-foreground uppercase tracking-wider">{t("market.total_supply")}</div>
                  <div className="font-bold mt-1 font-mono text-foreground">{selectedToken.total_supply.toLocaleString()}</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/30 border border-border">
                  <div className="text-xs text-muted-foreground uppercase tracking-wider">{t("portfolio.balance")}</div>
                  <div className="font-bold mt-1 font-mono text-foreground">
                    {holdings.find(h => h.token_id === selectedToken.id)?.amount.toFixed(2) || "0.00"}
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          {/* Price Chart */}
          <PriceChart currentPrice={selectedToken.current_price} ticker={selectedToken.ticker} tokenId={selectedToken.id} />
        ...
        <div className="lg:col-span-4 space-y-4">
            ...
            <TabsContent value="usd" className="space-y-3 mt-3">
              ...
              <Card className="border-primary/20">
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button onClick={() => handleOrder("buy")} className="w-full bg-primary hover:bg-primary/90 h-9">
              ...
              <Card className="border-orange-500/20">
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button onClick={() => handleOrder("sell")} className="w-full bg-orange-500 hover:bg-orange-600 h-9">
            ...
            <TabsContent value="swap" className="space-y-3 mt-3">
              <Card className="border-primary/20">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs text-muted-foreground">{t("trading.swap.subtitle", { ticker: selectedToken.ticker })}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div>
                    <Label className="text-primary font-bold text-xs">{t("trading.swap.want_receive")}</Label>
                    <div className="flex gap-2 mt-1">
                      ...
                      <div className="px-3 py-2 bg-primary/10 border border-primary/20 rounded-md flex items-center">
                        <span className="font-mono font-bold text-primary text-xs">{selectedToken.ticker}</span>
                      </div>
                    </div>
                  </div>
                  <div>
                    <Label className="text-foreground text-xs">{t("trading.swap.pay_with")}</Label>
                    <div className="flex gap-2 mt-1">
                      <Input
                        type="number"
                        value={payAmount || ""}
                      ...
                      <Select value={paymentTokenId} onValueChange={setPaymentTokenId}>
                        ...
                        <SelectContent>
                          {myPaymentHoldings.length === 0 && <SelectItem value="none" disabled>{t("trading.swap.no_tokens")}</SelectItem>}
                  ...
                  {receiveAmount > 0 && payAmount > 0 && (
                    <div className="p-2 bg-muted/30 rounded-lg border border-border/50 text-xs">
                      <span className="text-muted-foreground">{t("trading.swap.exchange_rate")}:</span>
                      <span className="ml-2 font-mono text-primary">
                        1 {selectedToken.ticker} ≈ {(payAmount / receiveAmount).toFixed(4)}{" "}
                        {myPaymentHoldings.find(h => h.token_id === paymentTokenId)?.tokens?.ticker}
                      </span>
                    </div>
                  )}
                  <Button onClick={handleSwapOffer} className="w-full bg-primary hover:bg-primary/90 h-9">
          ...
          <div className="space-y-4">
            <Tabs defaultValue="orderbook" className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-muted h-10">
                <TabsTrigger value="orderbook" className="text-xs">{t("trading.order_book.title")}</TabsTrigger>
                <TabsTrigger value="history" className="text-xs">{t("trading.history.title")}</TabsTrigger>
              </TabsList>
                  ...
                  <CardContent className="p-0">
                    <div className="max-h-[300px] overflow-y-auto pt-2 px-2 pb-2">
                      ...
                      <div className="space-y-1">
                        {orders.length === 0 ? (
                          <p className="text-center text-muted-foreground text-xs py-8">{t("trading.no_active_orders")}</p>
                        ) : (
                          orders.slice(0, 15).map((order) => (
                            ...
                            >
                              <div className="flex items-center gap-1">
                                <span className={`w-1 h-8 rounded-sm ${order.type === 'buy' ? 'bg-green-500/50' : 'bg-red-500/50'}`}></span>
                              ...
                              <div className="text-right flex items-center justify-end gap-2">
                                ...
                                {order.user_id === user?.id ? (
                                  ...
                                  >
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><path d="M18 6 6 18"/><path d="m6 6 18 18"/></svg>
                                  </Button>
                                ) : (
                                  <Button size="icon" variant="ghost" className="h-5 w-5 rounded-full hover:bg-primary/20 hover:text-primary">
          ...
          {swapOffers.length > 0 && (
            <Card className="border-border/50">
              <CardHeader className="pb-3">
                <div className="text-sm font-display font-bold">{t("trading.swap.active_offers")}</div>
              </CardHeader>
              <CardContent className="max-h-[300px] overflow-y-auto">
                <div className="space-y-2">
                  {swapOffers.slice(0, 5).map((offer) => (
                    <div key={offer.id} className="p-2 bg-muted/30 rounded-lg border border-border/50 text-xs">
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-muted-foreground">{t("trading.swap.receives")}:</span>
                          <div className="font-mono text-sm text-primary">{offer.amount} {selectedToken.ticker}</div>
                        </div>
                        <ArrowRight className="w-4 h-4 text-muted-foreground" />
                        <div>
                          <span className="text-muted-foreground">{t("trading.swap.pays")}:</span>
                          <div className="font-mono text-sm">{offer.amount} {offer.payment_token?.ticker}</div>
                        </div>
                      </div>
                      <div className="flex gap-2 mt-2">
                        {offer.user_id !== user?.id ? (
                          <Button onClick={() => handleAcceptSwap(offer.id)} variant="outline" className="w-full h-7 text-xs flex-1">
                            {t("trading.swap.accept")}
                          </Button>
                        ) : (
                          <Button onClick={() => handleCancelOrder(offer.id)} variant="destructive" className="w-full h-7 text-xs flex-1 opacity-80 hover:opacity-100">
