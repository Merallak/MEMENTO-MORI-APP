...
export function Payments() {
  ...
  const loadMyTokens = async () => {
    ...
    const ownedTokens = holdings
      ...
      .filter((tok): tok is NonNullable<typeof tok> => tok !== null && tok !== undefined); // Filter out nulls

    const ownedTickers = ownedTokens.map((tok) => tok.ticker);

    const allTokens = await DataService.getAllTokens();
    const owned = allTokens.filter((tok) => ownedTickers.includes(tok.ticker));
  ...
  const generateQRCode = () => {
    ...
    const token = myTokens.find((tok) => tok.ticker === selectedToken);
    if (!token) return;
  ...
  const executePayment = async () => {
    ...
    try {
      ...
      if (!holding || holding.amount < scannedData.amount) {
        throw new Error(t("payments.insufficient_balance"));
      }
              ...
              >
                ...
                {myTokens.length === 0 ? (
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>{t("portfolio.no_holdings")}</AlertDescription>
                  </Alert>
                ) : (
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="token-select">{t("trading.select_token")}</Label>
                    ...
                    {qrData && (
                      ...
                      >
                        ...
                        <div className="text-center text-sm text-gray-600 space-y-1">
                          <p>
                            <strong>{t("market.ticker")}:</strong> {selectedToken}
                          </p>
                          <p>
                            <strong>{t("payments.amount")}:</strong> {requestAmount}
                          </p>
                        </div>
                      </motion.div>
                    )}
                  ...
                  >
                      ...
                      <div className="flex justify-between border-t border-border pt-2 mt-2">
                        ...
                        <span
                          className={`font-medium ${
                            senderBalance >= scannedData.amount
                              ? "text-green-500"
                              : "text-destructive"
                          }`}
                        >
                    ...
                    {paymentResult && (
                      ...
                      >
                        ...
                        <AlertDescription
                          className={paymentResult.success ? "text-green-500" : ""}
                        >
                    ...
                    {!paymentResult && (
                      <Button
                        ...
                        className="w-full"
                      >
                        {paymentProcessing ? t("common.loading") : t("common.confirm")}
                      </Button>
                    )}
                  </motion.div>
                )}
